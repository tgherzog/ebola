...
// run build routines
$url = 'https://' . $params['wfp']['host'] . '/api/3/action/package_show?id=' . $params['wfp']['id'];
$params['_src'] = $params['wfp']['host'] . '/dataset/' . $params['wfp']['id'];
...
// For now, we don't reshape the file at all, we just use the HDX data
$path = get_file($url, $params['wfp']['target_name']);
if( ! $path ) {
...
$data = $s->rangeToArray("A2:D$last", null, true, false);
foreach($data as $row) {
  if( ! $row[0] ) break;

  list($most_recent,$fed_people,$tot_supplies) = $row;
}
...
elseif( $params['status'] ) {
  ...
  $output = array(
    array('Affected People Receiving Food', $fed_people, sprintf('%dk', $fed_people/1000)),
	array('Amount of Total Supplies Dispatched', $tot_supplies, '$'. sprintf('%.1fm', $tot_supplies/1000000)),
  );

  if( $fed_people > 1000000 ) $output[0][2] = sprintf('%.1fm', $fed_people/1000000);
...
function ex_date($date, $fmt='m/j/Y') {
